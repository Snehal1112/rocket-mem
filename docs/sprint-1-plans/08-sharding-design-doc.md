# Sharding Design Doc Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** write up the sharding decision made in `02-sharded-keyspace.md` so the reasoning survives past whoever implemented it.

**Architecture:** this is a documentation-only plan — one file, one task. It documents *this sprint's concrete implementation choices* (16 shards, `DefaultHasher`-based routing); it doesn't re-litigate the higher-level architecture choice (sharded-lock vs. the 4 alternatives), which already lives in the master plan's Architecture Decision Record and should just be linked, not repeated.

**Depends on:** `02-sharded-keyspace.md` must be complete (the doc describes real, committed code).
...
**Files:**
- Create: `docs/design/sharding-decision.md`
...
git commit -m "docs: write up the sharding decision (16 shards, DefaultHasher routing)"
```
