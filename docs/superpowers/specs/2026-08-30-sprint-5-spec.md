...
**Atomicity requirement, part two — the follower's apply loop must take that same lock:** the argument above holds only against writers that reach the engine through `dispatch_and_log`, which acquires `lock_for_ordering()` before it mutates anything. A *follower* does not: it applies its leader's stream through plain `dispatch()` (per the follower decision below — a follower keeps no AOF, so it has nothing to log), and that path takes no lock at all. `SAVE` is deliberately permitted on a follower — snapshotting a follower's current state is a legitimate operator action, per the read-only gate decision below — so on a follower the two paths genuinely can interleave: `Store::snapshot_entries()` walks the 16 shards one at a time, and a replicated multi-key command (`MSET`, `RENAME`, `SINTERSTORE`, anything touching more than one key) mutates more than one shard with no single lock spanning it, so a walk can capture some of that command's keys at their new value and the rest at their old one. That is a **torn** snapshot: not merely stale, but internally inconsistent in a way no single point in the leader's history ever was, and a future `aof::recover` would load it without complaint. This is a distinct failure from the double-apply above — it needs no AOF tail at all to go wrong — and it is not one of this sprint's accepted limitations.

**Decision:** the follower apply loop (`sync_once` in `crates/server/src/replication.rs`) acquires `aof.lock_for_ordering()` around each frame's `dispatch()` call, and around nothing else — not the socket read, not the generation check — mirroring `handle_save`'s own discipline of holding the lock across the mutating work only. Because both sides now take the *same* lock, a `SAVE` and a replicated multi-key write on one node can only serialize. The `AofWriter` reaches the loop through a new `ReplicationHandle::with_aof(Arc<AofWriter>)` builder method rather than a third `ReplicationHandle::new` parameter, so the ~25 existing `new` call sites (all of them tests, none of which run a `SAVE` against a follower) stay untouched; the field it sets is an `Option<Arc<AofWriter>>`, left `None` by `new`/`Default`, and only `main.rs` calls `with_aof` — handing over the same `AofWriter` `serve()` receives, which is precisely what makes the two lock users the same lock. Nothing here changes the leader side: a leader's every write already funnels through `dispatch_and_log`.
...
`SAVE` is **not** added to `WRITE_COMMANDS` (`crates/server/src/aof.rs`) — it doesn't mutate the keyspace, so it has nothing for the AOF to log, and it must still be permitted on a read-only follower (an operator may want to snapshot a follower's current state) — the read-only gate decision below only rejects commands in `WRITE_COMMANDS`. That permission is exactly what makes the follower apply loop's own `lock_for_ordering()` load-bearing; see the "part two" atomicity decision above.
